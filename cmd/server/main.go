...
func main() {
	...
	api := r.Group("/api")
	{
		...
		auth.Use(middleware.Auth(authSvc))
		{
			...
			auth.POST("/summary-templates", summaryTemplateHandler.Create)
			auth.PUT("/summary-templates/:id", summaryTemplateHandler.Update)
			auth.DELETE("/summary-templates/:id", summaryTemplateHandler.Delete)
